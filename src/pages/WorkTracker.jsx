...
const WorkTracker = () => {
  ...
  const calculateMostAndLeastTimeSpent = () => {
    ...
    subjects.forEach((sub) => {
      ...
      const totalTimeLeast =
        leastTimeSpent.minutes * 60 + leastTimeSpent.seconds;
      ...
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col items-center justify-center p-4">
        <div className=" bg-gray-100 dark:bg-gray-900 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-20 p-7">
            ...
            <div className="flex justify-center space-x-4 mb-6">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className="border ${
                    theme === 'dark' ? 'text-white' : 'text-black'
                  } border-gray-300 rounded-md px-3 py-2"
                  placeholder="00"
            ...
            <div className="flex justify-center space-x-4 font-mono">
              <button
                ...
                className={`bg-blue-500 hover:bg-blue-600 ${
                  theme === "dark" ? "text-white" : "text-black"
                } font-semibold px-4 py-2 rounded-md transition duration-200 ${
                  !isTimeSet ? "opacity-50 cursor-not-allowed" : ""
                }`}
                disabled={!isTimeSet}
              >
          ...
          <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-8 mt-6 w-full max-w-md">
            ...
            <h2 className="text-xl font-bold text-center dark:text-gray-200 mb-4">
              Tracked Subjects
            </h2>
            {subjects.length === 0 ? (
              //<p className="text-center ">No subjects logged yet.</p>
              <h2 className="text-xs font-bold text-center dark:text-gray-200 mb-">
              ...
              <ul className="space-y-2">
                {subjects.map((sub, index) => (
                  <li
                    key={index}
                    className="text-center font-semibold dark: text-gray-200"
                  >
            ...
            {showAnalysis && (
              <div className="mt-8 w-full max-w-4xl bg-white dark:bg-gray-800 shadow-md rounded-lg p-8">
                <h2 className="text-2xl font-bold text-center dark:text-gray-200 mb-4">
                  Analysis
                </h2>

                {/* Most and Least time spent subjects */}
                <div className="mt-4">
                  <h3 className="text-xl dark:text-gray-200 font-semibold">
                    Most Time Spent on:
                  </h3>
                  <p>
                    {mostTimeSpent.subject || "N/A"} - {mostTimeSpent.minutes}{" "}
                    minutes {mostTimeSpent.seconds} seconds
                  </p>

                  <h3 className="text-xl font-semibold dark:text-gray-200 mt-4">
                    Least Time Spent on:
                  </h3>
                  <p>
                    {leastTimeSpent.subject || "N/A "} -{" "}
                    {leastTimeSpent.minutes} minutes {leastTimeSpent.seconds}{" "}
                    seconds
                  </p>
                </div>
              </div>
